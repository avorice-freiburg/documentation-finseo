...
import { Download, Share2, FileCode, Upload, X, Bot, BarChart3 } from "lucide-react";
...
const CHART_PLACEHOLDERS: Record<ChartType, string> = {
  ...
  pareto: "Describe Pareto analysis data, e.g., 'Customer complaints by category showing 80/20 distribution'",
...
export default function ParetoChartGenerator() {
  const [prompt, setPrompt] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [chartConfig, setChartConfig] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);
  const [chartType, setChartType] = useState<ChartType>('pareto'); // Pre-selected to pareto
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Could not generate your Pareto chart. Please specify your data and analysis more clearly.');
      }

      setChartConfig(config);
    } catch (err) {
      console.error('Chart generation error:', err);
      setError('Could not generate your Pareto chart. Please specify your data and analysis more clearly.');
    } finally {
      setIsLoading(false);
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen w-full">
        ...
        <div className="text-center max-w-3xl mx-auto px-6 z-10 pt-16">
          <div className="mx-auto flex justify-center">
            <p className="border border-border bg-accent rounded-full text-sm h-8 px-3 flex items-center gap-2 mb-6">
              ...
              AI-Powered Pareto Chart Generator
            </p>
          </div>
          <h1 className="text-3xl md:text-5xl font-medium tracking-tighter mb-4">
            Create Pareto Charts with AI
          </h1>
          <p className="text-muted-foreground text-base md:text-lg">
            Generate professional Pareto charts to identify the vital few factors that drive 80% of your results. 
            Perfect for quality analysis, problem-solving, and data-driven decision making.
          </p>
        </div>
      </section>
      
      {/* Chart Generator Form */}
      <section id="pareto-chart-generator" className="w-full max-w-4xl mx-auto px-6 -mt-10 mb-16 z-10">
        <div className="rounded-xl bg-card shadow-xl border border-border overflow-hidden">
          <div className="p-6 md:p-8">
            <h2 className="text-2xl font-semibold mb-6 tracking-tight">Generate Your Pareto Chart</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <Sheet open={isDataSheetOpen} onOpenChange={setIsDataSheetOpen}>
                    ...
                    <SheetContent side="right" className="w-full sm:max-w-lg">
                      <SheetHeader>
                        ...
                        <SheetDescription>
                          Upload a CSV file to use as data for your Pareto chart
                        </SheetDescription>
                      </SheetHeader>
              ...
              <div>
                <Label htmlFor="chartDescription">Pareto Chart Description</Label>
                <Input
                  id="chartDescription"
              ...
              >
                {isLoading ? 'Generating...' : 'Generate Pareto Chart'}
              </Button>
            </form>
          </div>
      ...
      <AlertDialog open={showEmbedDialog} onOpenChange={setShowEmbedDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Pareto Chart Saved Successfully!</AlertDialogTitle>
            <AlertDialogDescription>
              Your Pareto chart has been saved and is ready to embed. Use the code below to embed this chart on your website:
            </AlertDialogDescription>
          </AlertDialogHeader>
